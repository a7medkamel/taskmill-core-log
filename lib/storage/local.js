...
const local_url = url.format({
    protocol  : config.get('log.protocol')
  , hostname  : config.get('log.hostname')
  , port      : config.get('log.port')
});
...
class Storage { 
  ...
  write(headers, stream) {
    ...
    // console.log('going to write headers', headers, this.body_name, this.head_name);
    var metadata = {
        headers       : _.omit(headers.toObject(), 'x-tm-status-code')
      , 'status-code' : headers.get('x-tm-status-code')
    };
    ...
    var ret =  Promise
                .all([
                  ...
                  , Promise.promisify(fs.outputJson)(this.head_name, metadata)
                ]);
