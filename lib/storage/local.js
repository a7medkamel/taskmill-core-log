...
class Storage { 
  ...
  write(headers, stream, options) {
    ...
    var ret =  Promise
                    ...
                    Promise.fromCallback((cb) => {
                      ...
                      to.once('open', (fd) => {
                        to.once('close', () => {
                          ...
                          retry(() => Promise.promisify(fs.stat)(name), { timeout: 1000, interval: 100, backoff: 2, max_interval : 400 }).asCallback(cb);
                        });
                      });
                ...
                // .then(() => {
                //   console.log('confirmed readability', this.id);
                // })
                .finally(() => {
                  // console.log('deleting write lock', this.id);
